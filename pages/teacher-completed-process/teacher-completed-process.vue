<template>
    <view>
        ...
        <navigation-bar title="已办流程" :back="true" color="white" background="#3a7bd5"></navigation-bar>
...
export default zpMixins.extend({
    ...
    methods: {
        ...
        loadApplications() {
            try {
                const currentTeacher = this.teacherInfo;
                    ...
                    if (isMyApplication) {
                        ...
                        formattedApp.statusText =
                            app.status === 'approved'
                                ...
                                ? '待管理员审核'
                                : '未知状态';
                        formattedApp.submitTimeText = this.formatTime(app.submitTime);
                        formattedApp.reviewTimeText = app.reviewTime ? this.formatTime(app.reviewTime) : '';
                        ...
                        formattedApp.timeSlot = app.timeSlot || (app.startTime && app.endTime ? `${app.startTime}-${app.endTime}` : '');
                        formattedApp.remark = app.purpose || app.remark || '';
                        formattedApp.statusText = this.getStatusText(app.status);
                        formattedApp.submitTimeText = this.formatTime(app.submitTime);
                        formattedApp.reviewTimeText = app.reviewTime ? this.formatTime(app.reviewTime) : '';
        ...
        updateStatistics() {
            ...
            // 只统计学生申请，不包括教师自己的申请
            const studentApplications = allApplications.filter((app: any) => !app.isMyApplication);
        ...
        updateFilterCounts() {
            ...
            const filterTabs = this.filterTabs.map((tab) => {
                ...
                if (tab.key === 'mine') {
                    // 我的：显示当前教师的所有申请
                    count = allApplications.filter((app: any) => app.isMyApplication).length;
                } else if (tab.key === 'student_approved') {
                    // 学生：已通过 - 包括已通过和待管理员审核的记录
                    count = allApplications.filter((app: any) => app.isStudentApplication && (app.status === 'approved' || app.status === 'teacher_approved')).length;
                } else if (tab.key === 'student_rejected') {
                    // 学生：已驳回 - 显示所有被驳回的记录
                    count = allApplications.filter((app: any) => app.isStudentApplication && app.status === 'rejected').length;
                }
                return {
                    ...tab,
                    count
        ...
        filterApplications() {
            ...
            if (currentFilter === 'mine') {
                ...
                filteredApplications = allApplications.filter((app: any) => app.isMyApplication);
            ...
            } else if (currentFilter === 'student_approved') {
                // 学生：已通过 - 显示已通过和待管理员审核的记录
                filteredApplications = allApplications.filter((app: any) => app.isStudentApplication && (app.status === 'approved' || app.status === 'teacher_approved'));
            } else if (currentFilter === 'student_rejected') {
                // 学生：已驳回 - 显示所有被驳回的记录（包括教师驳回和管理员驳回）
                filteredApplications = allApplications.filter((app: any) => app.isStudentApplication && app.status === 'rejected');
            }
            this.setData({
         ...
         * 新审核意见输入
         */
        onNewReasonInput(e: any) {
            this.setData({
                newReviewReason: e.detail.value
            });
        },
         ...
         */
        confirmReprocess() {
            const { newReviewReason, currentApplication, teacherInfo } = this;
            if (!newReviewReason.trim()) {
                uni.showToast({
                    title: '请填写审核意见',
                    icon: 'none'
                });
            ...
            try {
                ...
                if (applicationIndex !== -1) {
                    allApplications[applicationIndex] = {
                        ...
                        reviewerId: teacherInfo.teacherId || '',
                        reviewerName: teacherInfo.name || ''
                    };
